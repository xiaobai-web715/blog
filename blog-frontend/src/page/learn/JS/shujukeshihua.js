...
import '../../../mock/learn/JS/shujukeshihua'
import Css from '../../../assets/css/learn/JS/shujukeshihua.css'

const ShuJuKeShiHua = (props) => {
    const [data , setData] = useState([])
    const {value} = useSelector(state => state.groundColorReducer);
    let title = search(props.location.search).search.title;
    useEffect(() => {
        let isUnmounted = false;
        request(`${config.path}home/learn/JS/3` , 'get').then(res => {
    ...
    return (
        <div className={Css['page']}>
            <div className={Css['page-title']}>数据可视化</div>
            <div className={Css['img']}>
                <img src={require(`../../../assets/image/introduction/learn/${title ? title : '数据可视化'}.jpg`).default} alt=''></img>
            </div>
            ...
            <div className={Css['more']}>
                <Link to={config.path + `api/learn/more?id=JS/3`}><span className={(value  || sessionStorage.getItem('backgroundColor'))  === 'dark' ? Css['active'] : ''}>阅览更多&gt;&gt;</span></Link>
            </div>
        </div>
    )
}

export default ShuJuKeShiHua